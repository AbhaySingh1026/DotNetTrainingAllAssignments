...
END
ALTER PROCEDURE UPDATECUSTOMERFNAME @EMAIL VARCHAR(50),@FNAME VARCHAR(20) AS BEGIN UPDATE CUSTOMER SET FNAME = @FNAME WHERE EMAIL = @EMAIL END
ALTER PROCEDURE UPDATECUSTOMERLNAME @EMAIL VARCHAR(50),@LNAME VARCHAR(20) AS BEGIN UPDATE CUSTOMER SET LNAME = @LNAME WHERE EMAIL = @EMAIL END
ALTER PROCEDURE UPDATECUSTOMERPHONE @EMAIL VARCHAR(50),@PHONE bigint AS BEGIN UPDATE CUSTOMER SET PHONE = @PHONE WHERE EMAIL = @EMAIL END
ALTER PROCEDURE UPDATECUSTOMEREMAIL @EMAIL VARCHAR(50),@NEWEMAIL VARCHAR(50) AS
BEGIN
IF ((@NEWEMAIL LIKE '%[a-z,0-9]@[a-z]%.[A-Za-z]%[A-Za-z]') AND (@NEWEMAIL NOT LIKE '%@%@%'))
UPDATE CUSTOMER SET EMAIL = @NEWEMAIL WHERE EMAIL = @EMAIL
END
ALTER PROCEDURE DELETECUSTOMER @EMAIL VARCHAR(50) AS BEGIN DELETE FROM CUSTOMER WHERE EMAIL = @EMAIL END
ALTER PROCEDURE SHOWALLCUSTOMERS AS BEGIN SELECT * FROM CUSTOMER END
